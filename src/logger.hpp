...
public:
    ...
    public:
        ...
        Line& operator<<(const char* s) { append(s); return *this; }
        Line& operator<<(char c) { if (buf.size() < buf.max_size()) buf.push_back(c); return *this; }
        Line& operator<<(const etl::string<MAX_LOG_SIZE>& s) { append(s.c_str()); return *this; }
        template <size_t N>
        Line& operator<<(const etl::string<N>& s) { append(s.c_str()); return *this; }

        template <typename T>
        typename etl::enable_if<etl::is_integral<T>::value, Line&>::type
        operator<<(T v) {
            etl::string<32> tmp;
            etl::to_string(v, tmp);
            append(tmp.c_str());
        ...
        Line& operator<<(float v) { append_float(static_cast<double>(v)); return *this; }
        Line& operator<<(double v) { append_float(v); return *this; }
        Line& operator<<(long double v) { append_float(static_cast<double>(v)); return *this; }
    ...
    private:
        ...
        void append(const char* s) {
            if (!s) return;
            while (*s && buf.size() < buf.max_size()) buf.push_back(*s++);
        }

        void append_float(double v) {
            if (buf.size() >= buf.max_size()) return;
            if (v != v) { append("nan"); return; }
            if (v > 1e300) { append("inf"); return; }
            if (v < -1e300) { append("-inf"); return; }

            if (v < 0) { buf.push_back('-'); v = -v; if (buf.size() >= buf.max_size()) return; }
        ...
        void flush() {
            if (buf.empty()) return;
            if (SERCOM3_USART_WriteIsBusy()) { buf.clear(); return; }
    ...
    Line operator<<(const char* s) { Line p(*this); p << s; return p; }
    Line operator<<(char c) { Line p(*this); p << c; return p; }
    Line operator<<(const etl::string<MAX_LOG_SIZE>& s) { Line p(*this); p << s; return p; }
    template <size_t N>
    Line operator<<(const etl::string<N>& s) { Line p(*this); p << s; return p; }
    template <typename T>
    typename etl::enable_if<etl::is_integral<T>::value, Line>::type
    operator<<(T v) { Line p(*this); p << v; return p; }
    Line operator<<(float v) { Line p(*this); p << v; return p; }
    Line operator<<(double v) { Line p(*this); p << v; return p; }
    Line operator<<(long double v) { Line p(*this); p << v; return p; }
