...
AS5047P::RegisterData_t AS5047P::readDeviceRegister(RegisterAddress registerAddress) const {
        ...
        if (not SERCOM4_SPI_Write(&txBuffer[0], 2))
                return 0;
        ...
        if (not SERCOM4_SPI_Read(&rxBuffer[0], 2))
                return 0;
...
AS5047P::FieldMagnitude_t AS5047P::measureFieldMagnitude() const { return readDeviceRegister(RegisterAddress::MAG); }
